...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("remove_alliance")
        .setDescription("remove alliance from the list")
    ...
    async execute(interaction) {
        ...
        let list = await db.get(
            `guild.${interaction.guild.id}.alliance.${guildOrMember}`
        );
        if (!list || list.length === 0) {
            return interaction.reply("**There is no Alliance**");
        }
        let allianceListArr = [];
        for (const guild of await db.get(
            `guild.${interaction.guild.id}.alliance.${guildOrMember}`
        )) {
            allianceListArr.push({
                label: guild.name,
                description: guild.full,
                value: `${guild.name}!!${guildOrMember}!!##${mention}##`,
            });
        }
        const allianceList = new ActionRowBuilder().addComponents(
            new StringSelectMenuBuilder()
                .setCustomId("remove_alliance")
                .setPlaceholder("Nothing selected")
                .addOptions(
                    allianceListArr.map((guild) => {
                        return {
                            label: guild.label,
